...
3. **LAN host-based routing (HTTP only)** to reduce open ports, simplify URLs, and eliminate LAN cookie-domain collisions without cookie rewriting.
4. **Port-based fallback** remains supported for non-HTTP protocols and for networks where mDNS is unreliable or disabled.
...
Host-based URLs are easier to remember (`immich.piccolo.local`) and can reduce the number of open LAN ports for HTTP apps (often to just 80/443).
...
- Keep Remote TLS simple with a single wildcard certificate (`*.example.com`), where possible.
- Allow LAN access that does not depend on router/DNS configuration (mDNS-first).
- Maintain a **port-based fallback** for reliability and for non-HTTP protocols.

### 3.2 Non-Goals

- Replace port-based routing for `protocol: raw` or `flow: tls` listeners on LAN.
...
**Remote base:** the Nexus-managed base domain (from remote config), e.g., `example.com`.

Portal remains accessible at the configured portal hostname (e.g., `portal.example.com`).
...
- `immich.example.com`
- `metrics-immich.example.com`

This uses a single DNS label (left-most) for both primary and secondary listeners, preserving compatibility with a single-level wildcard certificate on Remote (`*.example.com`).
...
- App names are already validated as DNS-safe; they are used verbatim in hostnames.
- Listener names used in hostnames MUST be DNS-safe. Options:
  1. Enforce DNS label validation for `listeners[].name` when `protocol` is HTTP/WebSocket, or
  2. Add a dedicated field (e.g., `listeners[].host_label`) that is DNS-safe and used for hostname construction.
...
**Resolution:**
1. If hostname matches portal hostname → route to portal server.
...
Piccolo MAY expose a shared LAN HTTP entrypoint:
- `0.0.0.0:80` (and optionally `:443` if/when local TLS is supported),
- route by `Host` header:
  - `piccolo.local` → portal
  - `<app>.<lan-base>` → app primary listener
  - `<listener>-<app>.<lan-base>` → specific listener

This mode removes the need for per-app public ports for HTTP/WebSocket listeners on LAN.
...
4. Keep legacy port-based URLs working indefinitely (at least through v1), with explicit documentation of cookie-domain caveats.
