...
**Rollback drain linger.** `RestoreConnection`'s underlying `AddAndActivateConnection` is fire-and-forget at the D-Bus level — it returns once the activation request is queued, **not** when NM finishes authentication. NM's `need-auth` callback for the rollback can fire seconds later, by which time `Connect` has returned and any synchronously-paired Drain has run. To cover this window, the rollback's Drain is scheduled via `time.AfterFunc(rollbackDrainLinger, ...)` where `rollbackDrainLinger = 30s` — same budget as `WaitForActivation` on the primary path. Cache entry survives `Connect`'s return and remains available for the rollback's asynchronous secret request. Cost: one orphan timer per rollback (callable safely after `Manager.Stop` since `Drain` only touches the cache map under `cacheMu`); rollback PSK lingers in heap for up to 30s after Connect returns (folded into A-Risk-1's exposure-window analysis).
...
| Cached secret in process memory could leak via coredump | Defense-in-depth: piccolod's `nmagent.New()` calls `setrlimit(RLIMIT_CORE, 0)` at construction time, disabling coredumps process-wide for the lifetime of piccolod regardless of systemd unit settings. The systemd unit is *expected* to also set `LimitCORE=0` (defense in depth), but the unit ships from the OS image, not this repo. If both are bypassed (operator overrides cur/max ulimit), the secret is exposed for the Connect-in-flight window. |
