...
strict mounted-root validation are complete. The exact local candidate RPM
passed install-from-start-limit recovery, both terminal boot-health branches
with real reboots, and unattended locked-to-unlocked recovery; the authoritative
OBS package passed the live 2 GiB task-Critical replacement and unlock gate.
Field evidence from a software-TPM Raspberry Pi 400 subsequently reopened the
kill-policy gate: `KillMode=control-group` terminated `swtpm` before graceful
Stop Phase 0 could create a fresh handoff. The corrected policy is
`KillMode=mixed`; its post-release service-restart, subsequent-reboot,
task-Critical, and watchdog requalification is pending. The first reboot that
activates this corrected MicroOS snapshot may still require one manual unlock
because shutdown is owned by the previously active
`KillMode=control-group` unit; that transition is an accepted one-time rollout
exception, not evidence against the corrected policy.
Release qualification is not closed: the clean-cohort app/owner matrix, 4 GiB
profile, installed-record capacity baseline, repeated p95, soak, and canary
...
thresholds scale with it. `KillMode=mixed` keeps Piccolod-owned helpers
available while the main process runs its bounded graceful shutdown, then
applies the final kill to every process still in the service cgroup after main
exits or the stop timeout expires. Old direct children therefore cannot overlap
the replacement, while child-dependent shutdown work such as the software-TPM
auto-unlock handoff can complete. App workloads remain in their dedicated user
...
On a software-TPM appliance, post-release qualification begins without an
existing handoff and records every PID in the old service cgroup, including
Piccolod and `swtpm`. A service restart must create and consume a fresh handoff,
reach unlocked Ready without manual input, and leave every recorded old PID
absent. Run it only after the corrected snapshot is active and the accepted
one-time transition unlock is complete; the subsequent reboot must also unlock
without manual input. Task-Critical and watchdog fault injection repeat the
old-child cleanup check with the effective `KillMode=mixed` policy.
...
- `TasksMax=15%`, `WatchdogSec=60s`, `Restart=always`, `RestartSec=5s`,
  `KillMode=mixed`, the bounded invocation-idempotent post-stop marker
...
satisfy both obligations. Plan verdict: **GREEN**. The remaining live release-
qualification proof remains blocking.
...
DNS seed before rebuilding all image profiles. The strict mounted-root validator
subsequently passed against corrected VirtualBox Build21.1. The remaining live
qualification is not implied by these publication and mounted-image facts.
...
| Production unit and validators | Finite 15% task limit, 60-second service watchdog, always/5-second restart, mixed graceful-main/final-cgroup kill policy, bounded `ExecStopPost` recurrence marker, three-start/fifteen-minute limit, conditional PID-1 reboot composition, and upgrade-time stop/reset/start recovery in authoritative package, mounted image, and live boot | Published OBS `home:atdexterslab/piccolod` revision 78, companion recovery unit/helper/test, spec, and `0.2.40-1.1` binaries; `Makefile`; `scripts/systemd/piccolod-start-limit-recovery-test.sh`; `scripts/alpha/dev-vm-alpha-test.sh`; Piccolo OS `scripts/validate-image-policy.sh` at commit `965711b8`; corrected OBS image revision 21 and Build21.1 mounted-root result; local RPM `%check`; exact candidate-RPM alpha lifecycle and real reboot logs; authoritative-package task-Critical alpha result | reopened | The historical package/image evidence used `KillMode=control-group` and remains exact evidence for that artifact, not for the corrected policy. Re-close this row only after authoritative packaging carries `KillMode=mixed` and focused software-TPM graceful restart, task-Critical, and watchdog runs prove fresh continuity plus old-child cleanup. |
...
This paragraph records the historical candidate exactly as tested. Its
existing-handoff pickup did not exercise a fresh software-TPM shutdown
ceremony, so it does not qualify the corrected `KillMode=mixed` policy.
...
subsequent publication evidence below proves the remote artifact identity and
strict mounted image, but not its direct fault injection, a clean-image app
...
artifact identity and build-composition proofs.

From the Piccolo OS repository, the corrected artifact was then validated with:

```sh
sudo -A ./scripts/validate-obs-image.sh \
  --profile VirtualBox \
  --arch x86_64 \
  /tmp/piccolo-os.x86_64-0.2.0-VirtualBox-Build21.1.vdi.xz
```

The read-only mounted-root validator exited zero with `PASS (132 checks, 4
warnings)`. It inspected the effective Piccolod and recovery-companion systemd
configuration, package versions, health-checker composition, bootstrap DNS,
signing/repository policy, and the remaining strict final-filesystem policy.
The four warnings were the validator's existing optional-file skips for absent
DHCP hostname policy and three absent `zypp.conf` policies; none was a required
artifact check.

### Authoritative-package task-Critical alpha evidence — 2026-07-22

The signed x86_64 OBS package above was copied to the exclusive 2 GiB,
one-vCPU, no-swap alpha VM. Its SHA-256 matched the published artifact and both
header and payload RSA/SHA256 signatures verified with the repository-owned
Piccolo signing key before installation. The older alpha image required its
missing `wireless-regdb` dependency to be installed first; corrected Build21.1
already proves final-image dependency resolution separately. The resulting
live package was `piccolod-0.2.40-1.1.x86_64`, unlocked and HTTP Ready with the
effective 2294-task limit, five-second restart delay, one-minute watchdog,
control-group kill, and recovery-companion handoff.

A bounded injector placed 1250 sleeping threads inside
`piccolod.service`, raising the complete cgroup from 10 to 1261 tasks. Two high
samples produced Warning with `reason_code=high_water`; `/api/v1/health/ready`
continued to return HTTP 200 while admission was fenced. After the specified
60-second window, the 2026-07-22 16:38:07 IST sample became Critical with
`reason_code=sustained_high_water`. The census attributed 1251 threads to the
injector and 10 to Piccolod, the marker preserved the exact 1261/2294 sample and
failed invocation `79b239c447ec4d2aa07d8f295266f193`, continuity reported
`reused`, and Piccolod exited with status `75/TEMPFAIL`.

systemd began the replacement after its five-second delay. PID 7647 became PID
9563 with a new invocation; both the old daemon and injector PID were absent,
`NRestarts=1`, tasks returned to 10/2294, and `pids.events:max` remained zero.
The boot ID stayed `a6c039e0-466f-4ce7-8e41-486c99dd3e5e`, proving the normal
first-generation service recovery did not request an unnecessary host reboot.
Correlated stage telemetry measured 5.49 seconds from Critical detection to
core Ready and 6.74 seconds to both active unlock pickup and lifecycle Ready.
The device was automatically unlocked, readiness returned HTTP 200 with every
component OK, and the packaged post-setup smoke passed 6/6. With no installed
app route in this reused alpha, qualification correctly recorded
`listenerless_no_cohort`; route-bearing p95 and the clean app/owner matrix remain
separate pending gates. At 16:48:15 IST, after the required ten-minute stable
window, the runner emitted `eventual_convergence_complete`, removed the volatile
marker itself, and left the same replacement active with 10 tasks.
...
- **blocking × in-scope — release qualification evidence**
  ...
  - **Statement:** live 2 GiB unit policy, watchdog/task-critical recovery,
    ...
    branches with real reboots, unattended unlock recovery, authoritative
    package publication and direct task-Critical recovery, and strict mounted-
    image composition are proven. Release still lacks a clean-cohort app
    lifecycle, the named owner/transition interruption matrix, repeated 2 GiB
    and 4 GiB p95 timing, the D8 constant-overhead baseline, 72-hour mixed-
    operation soak, and canary telemetry. The monolithic Flutter gate remains
    unavailable because of pre-existing cross-platform harness incompatibility,
    although every compatible partition is green.
  - **Suggested resolution:** run the remaining live fault matrix on clean 2
    GiB and 4 GiB canaries; capture the installed-record, soak, and canary
