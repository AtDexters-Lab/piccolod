<script lang="ts">
  ...
  const baseSteps: StepDefinition[] = [
    ...
    { id: "credentials", label: "Credentials", description: "Unlock Piccolo" },
    {
      id: "recovery",
      label: "Recovery key",
      description: "Save the 24-word key",
    },
  ...
  const unsubscribePage = page.subscribe(($page) => {
    ...
    passwordRecoveryTarget = redirectProvided
  ...
  async function loadRecoveryStatus() {
    ...
    try {
      ...
      recoveryStatusError =
        apiError?.message ?? "Unable to check recovery key status.";
    } finally {
  ...
  async function performRecoveryGeneration(): Promise<boolean> {
    ...
    try {
      ...
      const apiError = error as ApiError | undefined;
      recoveryStatusError =
        apiError?.message ?? "Failed to generate the recovery key.";
      return false;
    } finally {
  ...
  async function confirmRegenerate() {
    ...
    if (success) {
      ...
      regenerateError =
        recoveryStatusError || "Unable to regenerate the recovery key.";
    }
  }
  ...
  async function copyRecoveryKey() {
    ...
    try {
      ...
      recoveryCopyNotice =
        "Unable to copy automatically. Select the words and copy manually or download the file.";
    }
  }

  function downloadRecoveryKey() {
    if (!generatedRecoveryWords.length) return;
    const blob = new Blob([generatedRecoveryWords.join(" ")], {
      type: "text/plain",
    });
  ...
  $: needsServerRecovery =
    !recoveryStatus || !recoveryStatus.present || Boolean(recoveryStatus.stale);
  ...
  $: submittingStep =
    setupState.phase === "submitting" ? setupState.step : null;
  ...
  $: introCtaLabel =
    setupState.phase === "ready"
      ...
      : "Start setup";
  $: finishCtaLabel =
    redirectProvided && redirectTarget !== "/" ? "Continue" : "Go to dashboard";
  ...
  async function handleSubmit(event: SubmitEvent) {
    ...
    if (mode === "unlock" && !unlockInputProvided()) {
      localError =
        "Enter the admin password to unlock Piccolo. Use your recovery key on the reset page if needed.";
      return;
    }
  ...
  function finish() {
    window.location.href = resolveRedirect("finish");
  }
  ...
  async function clearFocusQuery() {
    ...
    await goto(`${url.pathname}${search}${url.hash}`, {
      replaceState: true,
      noScroll: true,
    });
  }
  ...
  async function handleRecoveryContinue(
    ...
    if (!useExisting) {
      if (!generatedRecoveryWords.length || !recoveryAcknowledged) {
        recoveryCopyNotice =
          "Generate and save the recovery key before continuing.";
        return;
      }
      recoveryCopyNotice = "";
    ...
    try {
      ...
      const apiError = error as ApiError | undefined;
      continueExistingError =
        apiError?.message ?? "Unable to continue with the existing key.";
      return;
    } finally {
...
<div class="flex flex-col gap-6">
  <section
    class="rounded-3xl border border-white/30 bg-white/80 backdrop-blur-xl p-6 elev-3 text-ink"
  >
    <div
    ...
    >
      <div class="space-y-3 max-w-2xl">
        ...
        <p class="text-sm text-muted">
          Piccolo uses one password to initialize and unlock encrypted volumes.
          Keep it secret—there’s only one admin per device.
        </p>
      ...
      <div
        class="rounded-2xl border border-white/40 bg-white/70 px-5 py-4 text-xs text-muted lg:max-w-sm"
      >
  ...
  <Stepper {steps} activeId={activeStep} />

  {#if setupState.phase === "error"}
    <div
      class="rounded-3xl border border-red-200 bg-red-50/90 px-5 py-4 text-sm text-red-900 elev-1 flex flex-col gap-3"
    >
      ...
      {#if setupState.retryAfter}
        <p class="text-xs text-red-800/80">
          Please wait {setupState.retryAfter} seconds before retrying.
        </p>
      {/if}
    </div>
  {/if}
  ...
  {#if activeStep === "intro"}
    <section
      class="rounded-3xl border border-white/30 bg-white/85 backdrop-blur-xl elev-2 p-6 flex flex-col gap-4"
    >
      <p class="text-sm text-muted max-w-2xl">
        Before continuing, confirm you have physical access to the device. This
        password unlocks remote publish, encrypted volumes, and recovery
        actions.
      </p>
      <ul
        class="rounded-2xl border border-slate-200 px-4 py-4 text-sm text-ink space-y-2"
      >
  ...
  {:else if activeStep === "credentials" && mode}
    ...
    >
      ...
      <div class="grid gap-4 md:grid-cols-2">
          ...
          {#if mode !== "first-run"}
            <p class="text-xs text-muted">
              Forgot it? <a
                ...
                href={passwordRecoveryHref}>Reset with your recovery key</a
              >
            </p>
          {/if}
        </div>
        {#if mode === "first-run"}
          <div class="flex flex-col gap-2">
            <label class="text-sm font-medium text-ink" for="confirm-password"
      ...
      <div
        class="rounded-2xl border border-slate-200 px-4 py-3 text-xs text-muted"
      >
        ...
        <ul class="mt-1 list-disc list-inside space-y-1">
          ...
          <li>
            If the password is lost, use the recovery page to reset it before
            unlocking.
          </li>
        </ul>
      </div>
      <div class="flex gap-3 flex-wrap">
        <Button
          variant="ghost"
          type="button"
          on:click={() => (showCredentials = false)}
          disabled={isSubmitting}
        >
          Back
        </Button>
  ...
  {:else if activeStep === "recovery"}
    <section
      class="rounded-3xl border border-white/30 bg-white/90 backdrop-blur-xl elev-2 p-6 flex flex-col gap-4 text-ink"
    >
      <p class="meta-label meta-label--lg">Recovery</p>
      <h2 class="text-2xl font-semibold">Save your recovery key</h2>
      <p class="text-sm text-muted">
        Piccolo only shows this key once. Store it somewhere offline before you
        continue.
      </p>
      ...
      {:else}
        {#if recoveryStatusError}
          <div class="flex flex-col gap-2">
            ...
            <Button
              ...
              size="compact"
              type="button"
              on:click={retryRecoveryStatusCheck}
            >
              Retry status
            </Button>
          </div>
        ...
        {#if generatedRecoveryWords.length}
          ...
          <div class="flex flex-wrap gap-3">
            <Button variant="secondary" type="button" on:click={copyRecoveryKey}
              >Copy words</Button
            >
            <Button
              variant="secondary"
              type="button"
              on:click={downloadRecoveryKey}>Download .txt</Button
            >
          </div>
          ...
          <div class="flex gap-3 flex-wrap mt-4">
            <Button
              ...
              disabled={!generatedRecoveryWords.length ||
        ...
        {:else if recoveryStatus?.present}
          <div class="flex flex-wrap gap-3">
            <Button
              variant="primary"
              type="button"
              on:click={openRegenerateModal}
            >
              Regenerate key
            </Button>
            {#if recoveryStatus?.stale}
              ...
              {#if continueExistingError}
                <p class="text-xs text-red-600 w-full">
                  {continueExistingError}
                </p>
              {/if}
            {/if}
        ...
        {:else}
          <p class="text-sm text-muted">
            Generate the 24-word key now. You can’t finish setup until it’s
            saved.
          </p>
          <div class="flex gap-3 flex-wrap">
            <Button
  ...
  {:else if activeStep === "done" && setupState.phase === "ready"}
    ...
    >
      ...
      <p class="text-sm text-muted">
        Setup finished. Continue to the dashboard or sign in again later.
      </p>
      <div class="flex gap-3 flex-wrap">
  ...
  <div class="modal-backdrop">
    <div class="modal-card">
      ...
      <p class="text-sm text-muted mt-2">
        This will invalidate the previous key. Save the new 24 words
        immediately—Piccolo only shows them once.
      </p>
      {#if regenerateError}
