...
<section
  class="app-layer"
  use:layerStateAction={{ entry: { id: layerId, kind: 'app' }, isForeground: isTopLayer }}
    ...
    <div class="app-window" aria-label={`${activeApp.displayName} window`}>
      <header class="window-chrome">
        ...
        <div class="flex items-center gap-1">
          <Button variant="ghost" size="icon" on:click={() => dispatch('openNewTab')} title="Open in new tab">
          ...
          <Button variant="secondary" size="icon" on:click={() => dispatch('switchApp')} title="Switch app">
             <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>
          </Button>
          <Button variant="ghost" size="icon" on:click={() => dispatch('close')} title="Close">
