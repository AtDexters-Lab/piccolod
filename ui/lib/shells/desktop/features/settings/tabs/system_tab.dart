      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              'You will be redirected to login once the system is back online.',
            ),
          ],
        ),
      );
              ...
              if (update != null) ...[
                ...
                Row(
                  children: [
                    ...
                    OutlinedButton(
                      onPressed: (controller.isLoading || isBusy)
                          ? null
                          : () => _showRollbackConfirmation(context),
...
class _InstallToDiskCardState extends State<_InstallToDiskCard> {
  ...
  Future<void> _checkBootMode() async {
    try {
      ...
      final bootMode =
          (onboarding as Map<String, dynamic>)['boot_mode'] as String?;
  ...
  Future<void> _fetchDisks() async {
    try {
      ...
      final rawDisks =
          (response as Map<String, dynamic>)['disks'] as List<dynamic>? ??
          <dynamic>[];
      setState(() {
            ...
            for (final disk in _disks)
              _SettingsDiskTile(
                ...
                onTap: _isInstalling
                    ? null
                    : () => setState(
                        () => _selectedDisk = disk['device'] as String?,
                      ),
              ),
          const SizedBox(height: 16),
          Row(
...
class _DiagnosticLogSection extends StatelessWidget {
  const _DiagnosticLogSection();

  @override
  Widget build(BuildContext context) {
    return const DiagnosticLogDownload(
      apiPath: '/api/v1/system/admin/diagnostic-log',
    );
  }
}
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            color: isSelected
                ? PiccoloTheme.cobalt600.withValues(alpha: 0.04)
                : null,
          ),
          child: Row(
            children: [
